...
def get_args_parser():
    parser = argparse.ArgumentParser(
        "ToST training and evaluation script", add_help=False
    )
    ...
    # Dataset parameters
    parser.add_argument(
        "--data-path", default="path/to/imagenet", type=str, help="dataset path"
    )
    parser.add_argument(
        "--data-set",
        default="CIFAR10",
    ...
    parser.add_argument(
        "--full_crop",
        action="store_true",
        help="use crop_ratio=1.0 instead of the default 0.875 (Used by CaiT).",
    )
    parser.add_argument(
        "--pretrained", default=None, type=str, help="Path to pre-trained checkpoint"
    )
    parser.add_argument(
        "--surgery",
        default=None,
        type=str,
        help="Path to checkpoint to copy the patch projection from. Can improve stability"
...
def main(args):
    if not hasattr(args, "dist_url") or args.dist_url is None:
        ...
        args.dist_url = (args.output_dir / f"{uuid.uuid4().hex}_init").as_uri()
    ...
    if args.surgery:
        checkpoint = torch.load(args.surgery, map_location="cpu", weights_only=False)
        checkpoint_model = checkpoint["model"]
        patch_embed_weights = {
    ...
    max_lr = 0.1
    linear_scaled_lr = min(
        args.lr * args.batch_size * utils.get_world_size() / 512.0, max_lr
    )
    ...
    try:
        if args.shared_folder:
            # if shared_folder is set, it means we are using submitit.
            # the output_dir will be set to:
            #   [shared folder / args.model / args.data_set]
            output_dir = Path(args.output_dir)
    except AttributeError:
        # if shared_folder is not set, we run locally.
        # the default output_dir will be:
        #   [./checkpoint / args.model / args.data_set]
        output_dir = Path(args.output_dir) / args.model / args.data_set
    ...
    print("Final Cholesky stats:", utils.cholesky_stats)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "ToST training and evaluation script", parents=[get_args_parser()]
    )
