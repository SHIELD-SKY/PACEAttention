...
import torch
from timm.data import dataset as timm_dataset
...
def build_dataset(is_train, args):
    ...
    if args.data_set == "MNIST":
        ...
        # Use a deterministic class-balanced subset for quick MNIST experiments.
        per_class = 500 if is_train else 100
        subset_seed = 3407
        generator = torch.Generator().manual_seed(
            subset_seed + (0 if is_train else 1)
        )
        ...
        for class_id in range(10):
            ...
            k = min(per_class, class_indices.numel())
            indices_per_class.append(class_indices[perm[:k]])
        ...
        dataset = timm_dataset.ImageDataset(root, transform=transform)
        nb_classes = 1000
        ...
        root = os.path.join(args.data_path, "train" if is_train else "test")
        dataset = Flowers102(root, transform=transform, download=True)
        nb_classes = 102
    elif args.data_set == "PETS":
        root = os.path.join(args.data_path, "trainval" if is_train else "test")
